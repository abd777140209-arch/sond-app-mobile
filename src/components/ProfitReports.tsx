...
export default function ProfitReports({
  ...
  const [localPrivacyMode, setLocalPrivacyMode] = useState<boolean>(isPrivacyMode);
  ...
  const fmt = (num: number) => {
    ...
    if (localPrivacyMode || isPrivacyMode) return '**** ' + symbol;
...
📑 مبيعات آجل (ديون): ${fmt(stats.totalDebtRevenue)}`;
  ...
  return (
    <div id="profit_reports_view" className="space-y-6 pb-12">
      ...
      <div className="p-3.5 sm:p-5 md:p-6 rounded-2xl border border-slate-200 bg-white shadow-sm space-y-3.5">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3">
            ...
            <div className="flex items-center gap-2.5">
              {settings?.storeLogoUrl ? (
                <img 
                  ...
                  className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl object-contain border border-slate-200 bg-white p-0.5 shadow-xs shrink-0" 
                />
              ) : (
              ...
              <div>
                <h1 className="text-sm sm:text-base md:text-lg font-bold text-slate-900">
                  التقرير البياني لأرباح المبيعات - {settings?.storeName || 'المنشأة'}
                </h1>
          ...
          <div className="flex items-center gap-2 w-full md:w-auto flex-wrap">
            <button
              ...
              className="flex-1 md:flex-none px-4 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 active:scale-95 text-white text-xs font-bold transition flex items-center justify-center gap-1.5 cursor-pointer shadow-md shadow-blue-500/20 no-print"
            >
            ...
            <button
              ...
              className="flex-1 md:flex-none px-3.5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 active:scale-95 text-white text-xs font-bold transition flex items-center justify-center gap-1.5 cursor-pointer shadow-md no-print disabled:opacity-50"
            >
        ...
        <div className="pt-2.5 border-t border-slate-100 flex flex-wrap items-center justify-between gap-2.5">
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            <button
              ...
              className="px-2.5 py-1.5 rounded-lg bg-emerald-50 hover:bg-emerald-100 text-emerald-700 text-xs font-bold transition flex items-center gap-1 cursor-pointer border border-emerald-200 active:scale-95 no-print"
            >
            ...
            {reportSubTab === 'maintenance' && (
              <button
                ...
                disabled={isExportingPDF}
                className="px-2.5 py-1.5 rounded-lg bg-purple-50 hover:bg-purple-100 text-purple-700 text-xs font-bold transition flex items-center gap-1 cursor-pointer border border-purple-200 active:scale-95 no-print disabled:opacity-50"
              >
                {isExportingPDF ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Download className="w-3.5 h-3.5 text-purple-600" />}
                <span>صيانة PDF</span>
              </button>
            )}
            ...
            <button
              ...
              className="px-2.5 py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold transition flex items-center gap-1 cursor-pointer border border-slate-200 active:scale-95"
            >
            ...
            <button
              ...
              className={`px-2.5 py-1.5 rounded-lg text-xs font-bold transition flex items-center gap-1 cursor-pointer border active:scale-95 ${
                localPrivacyMode || isPrivacyMode 
                  ? 'bg-amber-50 text-amber-700 border-amber-200' 
                  : 'bg-slate-100 text-slate-700 border-slate-200 hover:bg-slate-200'
              }`}
              title="إخفاء/إظهار الأرقام للحفاظ على الخصوصية أمام العملاء"
            >
              {localPrivacyMode || isPrivacyMode ? <EyeOff className="w-3.5 h-3.5 text-amber-600" /> : <Eye className="w-3.5 h-3.5 text-slate-600" />}
              <span>{localPrivacyMode || isPrivacyMode ? 'الأرقام مخفية' : 'وضع الخصوصية'}</span>
            </button>
          </div>
