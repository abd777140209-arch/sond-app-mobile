...
export default function InvoiceModal({ invoice, onClose, settings, customers }: InvoiceModalProps) {
  ...
  const handleBluetoothPrint = async () => {
    ...
    try {
      if (typeof navigator !== 'undefined' && 'bluetooth' in navigator) {
        ...
        if (device) {
          alert(`✅ تم الاقتران بطابعة البلوتوث الحرارية (${device.name || 'طابعة البلوتوث'}). جاري تحويل الفاتورة لملف PDF ومشاركتها...`);
          await handleExportPDF();
        }
      } else {
        await handleExportPDF();
  ...
  return (
    <div 
      id="invoice_modal_overlay" 
      onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}
      className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 backdrop-blur-sm p-4 print:bg-white print:absolute print:inset-0 cursor-pointer"
    >
        ...
        <div className="p-3 bg-slate-900 text-white flex justify-between items-center border-b border-gray-800">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              onClick={onClose}
              className="p-1.5 px-2.5 bg-rose-600/90 hover:bg-rose-600 text-white rounded-lg text-xs font-bold cursor-pointer transition flex items-center gap-1 active:scale-95 shadow-sm"
